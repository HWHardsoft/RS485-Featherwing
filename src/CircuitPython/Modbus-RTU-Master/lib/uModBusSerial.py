...
class uModBusSerial:
    ...
    def __init__(self, baudrate=9600, ctrl_pin=None):
        self._uart = busio.UART(board.TX, board.RX, baudrate=baudrate)

        if ctrl_pin is not None:
            # self._ctrlPin = Pin(ctrl_pin, mode=Pin.OUTPUT)
            self._ctrlPin = digitalio.DigitalInOut(ctrl_pin)
            self._ctrlPin.direction = digitalio.Direction.OUTPUT
            self._ctrlPin = False
        else:
            self._ctrlPin = None
        # self.char_time_ms = 11 // baudrate
