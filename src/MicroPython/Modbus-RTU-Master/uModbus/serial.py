...
class Serial:
    ...
    def __init__(self, uart_id, baudrate=9600, data_bits=8, stop_bits=1, parity=None, tx_pin=None, rx_pin=None, ctrl_pin=None):
        #self._uart = UART(uart_id, baudrate=baudrate, bits=data_bits, parity=parity, \
        #                  stop=stop_bits, timeout_char=2, pins=pins)
        if tx_pin is not None and rx_pin is not None:
            self._uart = UART(uart_id, baudrate=baudrate, bits=data_bits, stop=stop_bits, parity=parity, tx=tx_pin, rx=rx_pin)
        else:
            self._uart = UART(uart_id, baudrate=baudrate, bits=data_bits, stop=stop_bits, parity=parity)
